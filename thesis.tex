...
\label{sec:orgc8010e9}
\subsection{Research Motivation}
\label{sec:orgbd26818}
\begin{itemize}
\item \cite{lopes11_integ_elect_vehic_elect_power_system}
\end{itemize}
\subsection{Research Question}
\label{sec:org23e5dd0}
\subsection{Relevance}
\label{sec:org1cbfe00}

\section{Theoretical Background}
\label{sec:orgfe3480d}
\subsection{Electricity Markets}
\label{sec:org733627d}
...
accessed 19\textsuperscript{th} February, 2019\label{orgcb5bcf3}}, but most electronic
...
trading volume of over 234TWh in 2018\textsuperscript{\ref{orgcb5bcf3}}. Participants place asks and bids
...
\label{sec:org7243e80}
When a bid gets accepted, the fleet has to provide the committed charging
capacity during the whole market period \(h\). To minimize the risk of not being
able to provide the committed capacity, and consequently causing imbalance
costs, the available fleet charging power in a market period is defined as the
minimal available fleet charging power in the control periods that make up the
market period (\(N_t\)).
\begin{equation}
...
\(\fP{h + (H \times 7)}\) one week ahead, where \(H\) is the number of market
periods in a day (\(H\) = 96).
...
charging from the respective markets.  As mentioned in the previous section, the
...
\end{split}
\end{equation}
where \(P\) is the amount of power charged in kW. The first term corresponds to
the compensation the controller retrieves for keeping the balancing capacity
available, while the second term corresponds to the costs for charging the
activated balancing energy \(\Eb{h}\). Note that the critical energy price
\(\cep{}\!\in\!\Re\), can also take negative values, resulting in profits for the
fleet, while the critical capacity price \(\ccp{}\!\in\! \Re^+_0\) cannot be
negative, always resulting in profits for the fleet. The cost function for
charging from the intraday market is defined similarly:
\begin{equation}
\begin{split}
...
\end{split}
\end{equation}
where \(P\) is the amount of power charged in kW. Again, \(\cup{}\!\in\!\Re\) can
take negative and positive values, depending on the market situation, resulting
in either costs or profits for the fleet.

Finally, when the expected costs for charging from the balancing market one week
ahead \(\Cb{h + (H \times 7)}(\fPhat{h + (H \times 7)})\) are higher than the
costs of charging from the intraday market at the same market period \(\Ci{h + (H
\times 7)}(\fPhat{h + (H \times 7)})\), the controller does not place bids in
Decision Phase I.
...
\end{itemize}
\subsection{FleetRL}
\label{sec:orge7ef88e}
\subsection{Sensitivity Analysis: Prediction Accuracy}
\label{sec:org8aa43ab}
\subsection{Sensitivity Analysis: Infrastructure Changes}
\label{sec:orgd06ca71}
\subsection{Sensitivity Analysis: Bidding Strategy}
\label{sec:org28809fb}
\section{Discussion}
\label{sec:org87d8866}
\subsection{Generalizability}
\label{sec:orgd5e4522}
\subsection{Future Electricity Landscape}
\label{sec:orga7da01b}
\subsection{Limitations}
\label{sec:orgfd2a337}
\section{Conclusion}
\label{sec:org3546457}
\subsection{Contribution}
\label{sec:org3d94517}
\subsection{Future Research}
\label{sec:org4ae731f}
