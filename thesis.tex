...
\label{sec:orgce21d8a}
\paragraph{Policy}
\label{sec:orgf060113}
\begin{itemize}
...
property is assumed throughout the Chapter \ref{sec-reinforcement-learning}.
...
If state and action spaces are continuous or the number of states and actions is
very high, a table representation computational infeasible and the speed of
convergence is drastically reduced. In this case, a \emph{function approximator} can
replace the lookup table. The next chapter will briefly cover approximation
methods, which are used to train such function approximators, as well as other
advancements in general reinforcement learning.
\subsubsection{Approximation Methods}
\label{sec:orgcd45557}
\paragraph{On-policy Prediction with Approximation}
\label{sec:orge900b47}
\begin{enumerate}
\item Linear Methods
\label{sec:orgaec2952}
\item Non-linear Methods: ANN
\label{sec:org8dde08e}
\end{enumerate}
\paragraph{(On-policy Control with Approximation)}
\label{sec:orga8bec39}
\clearpage
\section{Empirical Setting / Data (10\%)}
\label{sec:orgf64368e}
\subsection{Carsharing Fleets of Electric Vehicles}
\label{sec:org78b6397}
\subsubsection{Raw Data}
\label{sec:org198538e}
