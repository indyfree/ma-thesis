...
\label{sec:org3123863}
\subsection{Research Motivation}
\label{sec:org92b764d}
\begin{itemize}
\item \cite{lopes11_integ_elect_vehic_elect_power_system}
\end{itemize}
\subsection{Research Question}
\label{sec:org275dace}
\subsection{Relevance}
\label{sec:org22375d3}

\section{Theoretical Background}
\label{sec:orgd3015e4}
\subsection{Electricity Markets}
\label{sec:org27fc1ff}
...
essential for electricity markets \cite{kambil98_reeng_dutch_flower_auction}.
Electricity markets work according to the merit order principle in which
resources are considered in an ascending order of the energy price until the
capacity demand is met. The clearing price is determined by the energy price, at
the point where supply meets demand. Payment models differ in the markets: In
contrast to day-ahead markets, where a uniform pricing schema is applied, in
secondary reserve markets and intraday markets bidders get compensated by the
price they bid (pay-as-bid principle).
...
have to be made one week in advance when customer demands are still of
uncertain. In order to not face penalties for unfulfilled commitments only a
conservative amount of capacity can be offered to the market. On the other hand,
spot markets allow participants to continuously trade electricity products up to
fifteen minutes prior to delivery. At this point, it is possible to predict if
an EV is likely going to be rented out with a high accuracy. This creates the
possibility to trade the remaining available capacity with a low risk at the
spot market. In the following, we will explain the market design of balancing
markets and spot markets in more detail, since they are the markets we included
in our research.
\begin{figure}[htbp]
\centering
\includegraphics[width=.9\linewidth]{./fig/electricity_markets.png}
\caption[Electricity Market Design]{\textbf{Adapt own figure!} Interaction between electricity markets in relation to capacity allocation \label{fig-electricity-markets}}
\end{figure}

\subsubsection{Balancing Market}
\label{sec:org87f6777}
...
reserve that were previously mentioned. This thesis focuses on the secondary
operating reserve auction, in which participants must be able to supply or
absorb a minimum of 1MW of power over a 4 hour interval with a reaction time of
30 seconds.\footnote{See \url{https://regelleistung.net/}, accessed on 15\textsuperscript{th} February
2019, for further information on the market design and historical data.} Since EV batteries can absorb energy almost
instantly, when they are connected to a charging station, they are suitable to
provide such balancing services. Operating reserve providers have to be
qualified by the TSO to participate in the market and are able to reliably
provide the committed capacity. Although EV fleets are currently not qualified
by the GCRM to be used as operating reserve, they could theoretically handle the
minimum capacity requirements. Around 220 EVs would need to simultaneously
charge at standard 4.6kW charging stations to provide 1MW of downward regulating
capacity.
...
accessed 18\textsuperscript{th} February, 2019} The shorter auction cycles
facilitate the integration of renewable energy generators into the secondary
control reserve market, as they are dependent on accurate (short-term) capacity
forecasts.
...
markets (cited) \cite{pape16_are_fundam_enoug}. Next to the previously mentioned
balancing market at the end of the sequence, mainly two different types of spot
markets exists, the day-ahead market and the intraday market. The European Power
Exchange (EPEX Spot) is the largest electricity market in Europe, with a total
trading volume of approximately 567 TWh in 2018.\footnote{\url{https://www.epexspot.com/en/press-media/press/details/press/Traded\_volumes\_soar\_to\_an\_all-time\_high\_in\_2018},
accessed 19\textsuperscript{th} February, 2019} In Germany, the most
important spot market is the day-ahead auction with a volume of over 234 TWh.
The market closes at 12pm on the day before delivery, participants place asks
and bids for hourly contracts of the following day. The clearing price is
determined by this double auction, where supply meets demand, and all
participants are uniformly compensated that clearing price. The day-ahead market
plays an important role for the integration of RES, generators forecast the
expected generation capacity for the next day and sell those quantities in the
day-ahead market (cited) \cite{pape16_are_fundam_enoug}. After market closure, the
participants have the opportunity to trade the difference between the forecasted
day-ahead capacities and the more accurate intraday forecasts on the \emph{intraday
market}. This quantity called \emph{forecast error} is used by RES generators to
self-balance their portfolios. Participating on the intraday market is more cost
effective for participants than to rely on balancing services that have been
described in the previous chapter, where balancing posts are imposed
participants ex-ante \cite{pape16_are_fundam_enoug}. On the EPEX Intraday
Continuous market different products can be traded up until 5 minutes before
physical delivery. Hourly contracts, as well as 15-minute and block contracts
can be traded. In contrast to the day-ahead auction, the intraday market is a
continuous order-driven market. Participants can submit limit orders at any time
during the trading window and equally change or withdraw the order at any time
before the order is accepted. Limit orders are specified as quantity/price
pairs. When an order to buy (bid) and an order to sell (ask) is matched the
trade will immediately get executed. The order book is visible to all
participants, hence it is known which unmatched orders exist at the time of
interest. With a market volume of 82 TWh in 2018, the intraday-continous market
is of minor trading importance in Germany than the day-ahead market. Despite
that, it is of major importance to the stability of the grid, executed trades on
the intraday market potentially reduce activation of control reserve through the
TSO.

The continuous intraday market is attractive for Smart Charging EV fleets with
uncertain mobility demand. Due to the very short time before delivery, EV fleet
operators can rely on highly accurate forecasts of available battery capacity to
charge, before submitting an order to buy. In this way, they can reliably charge
at a potentially cheaper price at the intraday market than the standard tariff.
In an integrated bidding strategy, EV fleet operators can, similarly to RES
generators, balance out forecast errors of available battery capacity on the
intraday market. Bids that have been committed to other markets earlier (e.g.,
to the secondary operating reserve market)  can be complemented with trades on
the intraday market.

\begin{itemize}
\item \cite{kiesel17_econom_analy_intrad_elect_prices}
\item \cite{karanfil17_role_contin_intrad_elect_market}
\item \cite{pape16_are_fundam_enoug}
\end{itemize}



\subsection{Reinforcement Learning \label{sec-reinforcement-learning}}
\label{sec:org7733ba8}
