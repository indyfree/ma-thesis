...
\label{sec:org6713f95}
\subsection{Research Motivation}
\label{sec:org7f2df56}
\begin{itemize}
\item \cite{lopes11_integ_elect_vehic_elect_power_system}
\end{itemize}
\subsection{Research Question}
\label{sec:org11a61d2}
\subsection{Relevance}
\label{sec:orge9f39e0}

\section{Theoretical Background}
\label{sec:org190790e}
\subsection{Electricity Markets}
\label{sec:org38ed038}
...
accepted, instantly get compensated for the provided capacity: \(R_c = p_c \times P\).
At the time regulation capacity is needed (usually a day or a week later), the
TSO activates the capacity according to a merit order of the ascending \emph{working
prices} \(p_w\). Hence, providers are also compensated according to the actual
power they supplied: \(R_w = p_w \times P_a\), where \(P_a\) is the
supplied power at the timeslot. Since, provider get paid according to their
submitted price \(p_w\), this type of auction is a \emph{pay-as-bid} auction.

\subsubsection{Spot Market}
\label{sec:org933fd29}
\subsection{Reinforcement Learning \label{sec-reinforcement-learning}}
\label{sec:org3abc531}
