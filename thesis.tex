...
\label{sec:org525f0ff}
\paragraph{Policy}
\label{sec:org6c59027}
\begin{itemize}
...
actions are chosen infinitely often. On-policy methods improve the
...
\(\epsilon\)-greedy action selection. Figure \ref{fig-sarsa} depicts the process.
Off-policy methods improve the deterministic decision policy, by using a second
stochastic policy to generate behaviour. The first policy is becoming the
optimal policy by evaluating the exploratory behaviour of the second policy.
Off-policy approaches are considered more powerful than on-policy approaches,
and have a variety of additional use-cases. On the other side, they often have a
greater variance and take longer time to converge to an optimum.
...
\end{equation}
In this case, the estimated action values \(Q\) are updated towards the best
estimated action value of the next time step. In this way, \(Q\) directly
approximates the optimal action-value function \(q_*\), independently of the
policy the agent follows.


\begin{itemize}
\item off-policy (\emph{target} (deterministic) and \emph{behaviour} policies (stochastic, exploratory))
\end{itemize}

\subsubsection{Approximation Methods \label{sec-approximation}}
\label{sec:orgaba0c80}
\paragraph{On-policy Prediction with Approximation}
\label{sec:orgd06a8f5}
\begin{enumerate}
\item Linear Methods
\label{sec:org4c60068}
\item Non-linear Methods: ANN
\label{sec:org26a53d5}
\end{enumerate}
\paragraph{(On-policy Control with Approximation)}
\label{sec:orgd449144}
\clearpage
\section{Empirical Setting / Data (10\%)}
\label{sec:org6042183}
\subsection{Carsharing Fleets of Electric Vehicles}
\label{sec:orgcdff018}
\subsubsection{Raw Data}
\label{sec:org206a048}
