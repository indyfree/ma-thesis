...
\label{sec:org602f0f2}
\subsection{Research Motivation}
\label{sec:orgcb1e1c2}
\begin{itemize}
\item \cite{lopes11_integ_elect_vehic_elect_power_system}
\end{itemize}
\subsection{Research Question}
\label{sec:org7fea504}
\subsection{Relevance}
\label{sec:orgd38dea6}

\section{Background}
\label{sec:org3daebab}
\subsection{Smart Electricity Markets}
\label{sec:org3355de1}
...
accessed 19\textsuperscript{th} February, 2019\label{org1e6c327}}, but most electronic
...
trading volume of over 234TWh in 2018\textsuperscript{\ref{org1e6c327}}. Participants place asks and bids
...
\end{aligned}
\end{equation}
Solving this optimization problem with regular methods is a difficult task,
when we do not assume complete information of available charging power and
market prices. We chose to solve the problem, by using a RL learning approach
that is explained in Chapter \ref{sec-model-rl}.
...
\emph{16.08.2017-15:00}. First, it predicts that at that point in time, 250 EVs are
connected to a charging station, resulting in 900kW available fleet charging
power (\(\fPhat{h}\) = 900kW), when assuming a charging power of 3.6kW. Given the
critical prices \(\ccp{h}\) = 5\(\emw\), \(\cep{h}\) = -10\(\emwh\), and \(\cup{h}\) =
10\(\emwh\) for that market period, the controller now evaluates the cheapest
charging option. The flat industry electricity tariff is assumed to be \(p_i\) =
0.15\(\ekwh\). The costs for charging \(\fPhat{h}\) from the balancing market
\(\Cb{h}(900)\) = -6.25\(\eur\) are less than charging from the intraday market
\(\Ci{h}(900)\) = 2.25\(\eur\) or charging at the industry tariff: 900kW \(\times
\frac{\Delta h}{60} \times 0.15\ekwh\) = 33.75\(\eur\). In this example, the fleet
will even get compensated for charging from the balancing market. Second, the
controller has to determine the bidding quantity, therefore it evaluated a
possible risk  \(\lb{h} = 0.3\), of bidding on the balancing market, and sets the
bidding quantity to \(\Pb{h} = \fPhat{h} \times (1 - \lb{h})\) = 900 \(\times\) 0.7
= 630kW. When assuming that bids at the critical price, will always get
accepted, the controller procures 630kW from the balancing market and updates
its account with \(\Cb{h}(630)\) = -4.725\(\eur\).
...
\(\fPhat{h}\) = 810kW, for the same market period \emph{16.08.2017-15:00}, this time
...
and sets the bidding quantity to \(\Pi{h}\) = (810 - 630) \(\times\) (1 - 0.05) =
171kW.

\subsubsection{Dispatch Phase}
\label{sec:orgf774ba5}
\begin{itemize}
...
\clearpage
\section{Results}
\label{sec:org07bc16b}
\subsection{Simulation Settings}
\label{sec:orgcaeb20b}
\begin{itemize}
