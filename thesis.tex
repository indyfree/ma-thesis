...
\label{sec:orgafec9c6}
\subsection{Research Motivation}
\label{sec:org12e45c6}
\begin{itemize}
\item \cite{lopes11_integ_elect_vehic_elect_power_system}
\end{itemize}
\subsection{Research Question}
\label{sec:org8220806}
\subsection{Relevance}
\label{sec:org0af444d}

\section{Theoretical Background}
\label{sec:org212066a}
\subsection{Electricity Markets}
\label{sec:org0094545}
...
trading volume of over 234TWh in 2018\textsuperscript{\ref{orgde1106a}}. Participants place asks and bids
...
to five minute-ahead .


Similarly to the balancing market, the critical price \(\overline{p_u}\) has to be
determined for all trading intervals. \(\overline{p_u}\) is defined as the lowest
unit price of all executed trades \(\cal{T}\) in that interval:
\begin{equation*}
    \overline{p_u} \defeq \min_{t \in \cal{T}} p_u^t
\end{equation*}

Note that \(\overline{p_u}\) is the upper bound, it is the optimal bidding price.
\cite{brandt17_evaluat_busin_model_vehic_grid_integ}
Since trades are executed
immediately, when the unit price of an order to buy is equal or higher than the
unit price of an order to sell,
