...
\label{sec:orgec3cf0e}
\subsection{Research Motivation}
\label{sec:org306ed6e}
\begin{itemize}
\item \cite{lopes11_integ_elect_vehic_elect_power_system}
\end{itemize}
\subsection{Research Question}
\label{sec:org0f2deba}
\subsection{Relevance}
\label{sec:org3d96fab}

\section{Theoretical Background}
\label{sec:orga0dd155}
\subsection{Electricity Markets}
\label{sec:org23b2fc5}
...
bids in the following form to the market: \((P^{bal}, p_c, p_e)\), where \(P^{bal}\)
...
the provided capacity: \(R_c = p_c \times P^{bal}\). At the time regulation
capacity is needed, usually a day to a week later, the TSO activates the
capacity according to a merit order of the ascending \emph{energy prices} \(p_e\).
Hence, providers are also compensated according to the actual energy \(E^{bal}\)
they supplied or consumed: \(R_e = p_e \times E^{bal}\). Since provider get paid
...
accessed 19\textsuperscript{th} February, 2019\label{org9cc8260}}, but most electronic
...
trading volume of over 234TWh in 2018\textsuperscript{\ref{org9cc8260}}. Participants place asks and bids
...
specified as price-quantity pairs: \((P^{int}, p_u)\), where \(P^{int}\) is the
traded amount of electrical power in kW and \(p_u\) is the price for the delivered
...
profitably provide balancing services to the grid on multiple markets. The model
uses a RL agent that learns an optimal bidding strategy by interacting with the
electricity markets and reacts to changing rental demand of the EV fleet. A
control mechanism places bids on electricity markets, allocates available EVs to VPPs,
and intelligently dispatches EVs to provide the charging capacity. This chapter
is structured as follows: First, the information assumptions are listed, and the
general control mechanism is explained. Afterwards, the bidding strategy and the
RL approach are described in detail. Finally, the dispatch algorithm is
presented.

For the sake of conciseness and brevity, we formulate the problem as an
\emph{controlled EV charging} problem. The EV fleet operator represents the
controller, which aims to optimally charge the fleet by providing balancing
capacity. First, the controller predicts the amount of energy it can charge in a
given market period \(h\). The length of the market period \(\Delta h\) and the
market closing time depend on the considered electricity markets. Second, the
controller places bids on one or multiple markets to procure the predicted
amount of energy. Lastly at delivery time, the controller communicates with the
EV fleet to control the charging. Online EV control periods \(t\) are typically
shorter then market periods. In our case, the market periods of the e.g. the
EPEX Spot Intraday market is 15 minutes, while we consider EV control periods of
5 minutes. During the control phase, the controller has to take decisions on
fulfilling its commitments to the markets or compromising customer mobility.
...
\endlastfoot
\hline
\(P^{bal}\) & Amount of power offered on the balancing market. & kW\\
\(p_c\) & Capacity price. & \(\text{\euro{}/MW}\)\\
\(p_e\) & Energy price. & \(\text{\euro{}/MWh}\)\\
\(\overline p_c\) & Critical capacity price. & \(\text{\euro{}/MW}\)\\
\(\overline p_e\) & Critical energy price. & \(\text{\euro{}/MW}\)\\
\hline
\(P^{intr}\) & Amount of power to trade on the intraday market. & kW\\
\(p_u\) & Unit price. & \(\text{\euro{}/MWh}\)\\
\(\overline p_u\) & Critical unit price. & \(\text{\euro{}/MW}\)\\
\hline
\(t\) & Control period. & -\\
\(h\) & Market period. & -\\
...
\hline
\(Q^{fleet}_t\) & Amount of available fleet charging capacity in control period \(t\). & kWh\\
\(P^{fleet}_t\) & Amount of available fleet charging power in control period \(t\). & kW\\
\(\widehat{P}^{fleet}_t\) & Predicted amount of available fleet charging power in control period \(t\). & kW\\
\hline
\(E^{bal}_h\) & Amount of energy charged from balancing market in market period h. & MWh\\
\(E^{intr}_h\) & Amount of energy charged from the intraday market in market period h. & MWh\\
\hline
\hline
\end{longtable}

\subsection{Required Information Assumptions}
\label{sec:org48c8ee0}
In order to optimize the bidding strategy, the following information is assumed
to be available to the controller:
...
predict the amount of plugged-in EVs and consequently the fleet charging
power \(P^{fleet}_t\) of the fleet at control period \(t\). with increasing
certainty, one week, 24 hours and 30 minutes ahead. Past research presented
...
critical prices \(\overline p_c\), \(\overline p_e\), and \(\overline p_u\) for
each market period with high accuracy. Bids equal or below the critical price
will get accepted. Hence the critical prices form an essential piece of
information for the proposed bidding strategy. Electricity price forecasting
is an  extensively studied research area, with well-advanced prediction
algorithms \cite{weron14_elect_price_forec,avci18_manag_elect_price_model_risk}.
\end{enumerate}
Assuming available forecasting information is common practice in the
VPP and EV fleet charging literature
...
facilitate smart charging. The control mechanism is divided into three distinct
decision phases. In the first phase the controller\ldots{}
...
Other assumptions:
\begin{enumerate}
\item Bids on balancing market always get accepted (Known critical capacity price \(\overline{p_c}\)
...
a positive imbalance price \$\(\lambda\)\textsubscript{h}\^{}+ < \(p_u\). Because positive imbalance
...
\section{Simulation Platform: EV Sim}
\label{sec:org0286544}
\subsection{Event-based Simulation}
\label{sec:org53fe29d}
\subsection{Architecture / Components}
\label{sec:org24985a2}
\begin{figure}[htbp]
\centering
...
\label{sec:orgd6f99b2}

\section{Evaluation / Results}
\label{sec:org67e7b72}
\subsection{Benchmark: Ad-hoc Strategies vs FleetRL}
\label{sec:org8462950}
\begin{itemize}
...
\end{itemize}
\subsection{FleetRL}
\label{sec:orgcfe4b3d}
\subsection{Sensitivity Analysis: Prediction Accuracy}
\label{sec:org2297341}
\subsection{Sensitivity Analysis: Infrastructure Changes}
\label{sec:orgc15407f}
\subsection{Sensitivity Analysis: Bidding Strategy}
\label{sec:orgc4b173f}
\section{Discussion}
\label{sec:org7b95680}
\subsection{Generalizability}
\label{sec:org0c96575}
\subsection{Future Electricity Landscape}
\label{sec:orgea53b45}
\subsection{Limitations}
\label{sec:orgd1acd7e}
\section{Conclusion}
\label{sec:org3291b34}
\subsection{Contribution}
\label{sec:org84dd0c7}
\subsection{Future Research}
\label{sec:org8aaf0cc}
