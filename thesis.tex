...
\label{sec:org39ccb22}
\subsection{Research Motivation}
\label{sec:org8b1ff38}
\begin{itemize}
\item \cite{lopes11_integ_elect_vehic_elect_power_system}
\end{itemize}
\subsection{Research Question}
\label{sec:org9c0b96f}
\subsection{Relevance}
\label{sec:orgb0dff75}

\section{Theoretical Background}
\label{sec:orgf3ad76a}
\subsection{Electricity Markets}
\label{sec:orgac1314a}
...
accessed 19\textsuperscript{th} February, 2019} but most
...
trading volume of over 234TWh. Participants can place asks and bids for hourly
contracts of the following day on the EPEX Spot until the market closes at 12pm
on the day before delivery (Figure \ref{fig-electricity-markets}).

The day-ahead market plays an important role for the integration of RES, generators
forecast the expected generation capacity for the next day and sell those
quantities on the this market (cited) \cite{pape16_are_fundam_enoug}.

After the day-ahead market closure, the participants have the opportunity to
trade the difference between the forecasted day-ahead capacities and the more
accurate intraday forecasts on the \emph{intraday market}. This quantity called
\emph{forecast error} is used by RES generators to self-balance their portfolios.
Participating on the intraday market is more cost effective for participants
than to rely on balancing services that have been described in the previous
chapter, where balancing posts are imposed participants ex-ante
\cite{pape16_are_fundam_enoug}. On the EPEX Intraday Continuous market different
products can be traded up until 5 minutes before physical delivery. Hourly
contracts, as well as 15-minute and block contracts can be traded. In contrast
to the day-ahead auction, the intraday market is a continuous order-driven
market. Participants can submit limit orders at any time during the trading
window and equally change or withdraw the order at any time before the order is
accepted. Limit orders are specified as quantity/price pairs. When an order to
buy (bid) and an order to sell (ask) is matched the trade will immediately get
executed. The order book is visible to all participants, hence it is known which
unmatched orders exist at the time of interest. With a market volume of 82TWh in
2018, the intraday-continuous market is of minor trading importance in Germany
than the day-ahead market. Despite that, it is of major importance to the
stability of the grid, executed trades on the intraday market potentially reduce
activation of control reserve through the TSO.
