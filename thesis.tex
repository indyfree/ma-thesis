...
\label{sec:orgebef7ab}
\subsection{Research Motivation}
\label{sec:orge3bdc90}
...
\label{sec:org709de06}

Drawing upon the research motivation, this research aims to answer the following research questions:

\begin{enumerate}
\item \emph{Can EV fleet operators create VPP portfolios to trade electricity on the
balancing market and intraday market simultanously?} \emph{How does an integrated
bidding strategy look like, which considers this case?}

\item \emph{Can a reinforcement learning agent optimize VPP portfolios by dynamically
learning the risks that are associated with bidding on the individual}
\emph{electricity markets?}
\end{enumerate}

\subsection{Relevance}
\label{sec:orgc5e89de}
...
\clearpage
\section{Background}
\label{sec:orgeed5d66}
\subsection{Smart Electricity Markets}
\label{sec:org3e7be19}
...
accessed 19\textsuperscript{th} February, 2019\label{orgfd14b6b}}, but most electronic
...
trading volume of over 234TWh in 2018\textsuperscript{\ref{orgfd14b6b}}. Participants place asks and bids
...
\clearpage
\section{Results}
\label{sec:org3a3ce26}
\subsection{Simulation Environment}
\label{sec:org5608069}
...
EV range & 160 km & \\
Industry electricity price & 0.15\footnotemark \(\ekwh\) & \(p^{ind}\)\\
Car2Go Rental Tariff & 0.15\textsuperscript{\ref{orgf48afd1}} \(\eur\) / minute & \\
\hline
\hline
\end{longtable}
\footnotetext[11]{\label{orgf48afd1}Average industry electricity price for 2017 BMWi. (n.d.). Prices of
...
\begin{itemize}
\item No extra Car2Go fees after 50km, etc..?
...
EVs available (min, \textbf{avg}, max, \textbf{std}) & 165, \textbf{389.64}, 496, \textbf{49.18}\\
EVs charging (min, \textbf{avg}, max, \textbf{std}) & 34, \textbf{61.23}, 290, \textbf{61.11}\\
VPP EVs (min, \textbf{avg}, max, \textbf{std}) & 0, \textbf{13.84}, 94, \textbf{9.01}\\
\hline
\hline
\end{longtable}

\item Footnote: Run with Data from \ldots{} till \ldots{}
...
\subsection{Fleet Profits 1. RQ}
\label{sec:org4c31e07}
\begin{itemize}
\item 1. RQ: Monthly gross profit increase with benchmark strategy: Bar-Plot? (integrated naive
strategy)
\item Error Matrix? Predicted available EVs vs actually available --> Fixed by
...
\centering
\begin{tabular}{p{0.25\linewidth}|cccccc}
...
Gross profit increase (1000 \eur) & 0 & 43.62 & 45.08 & \textbf{67.04} & \textbf{72.51} & 99.13\\
\hline
\hline
\end{tabular}
\end{sidewaystable}
...
\subsection{RL Learning 2. RQ}
\label{sec:orgff9313c}
\begin{itemize}
\item long-delayed rewards make RL hard (!?)
\item positive reward from buying immediatly vs negative rewards up to 7 days.
...
\label{sec:org624e5bf}
\begin{itemize}
\item Minimal bidding quantities (100kw, 500kw, 1MW)
\end{itemize}

\section{Conclusion}
\label{sec:orgf3ee5f7}
\subsection{Contribution}
\label{sec:orgfc0d276}
\begin{itemize}
