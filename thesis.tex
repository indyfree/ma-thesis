...
\label{sec:orgf59497e}
\subsection{Research Motivation}
\label{sec:org89b3892}
\begin{itemize}
\item \cite{lopes11_integ_elect_vehic_elect_power_system}
\end{itemize}
\subsection{Research Question}
\label{sec:org418583f}
\subsection{Relevance}
\label{sec:orge188afe}

\section{Theoretical Background}
\label{sec:orgba14c0e}
\subsection{Electricity Markets}
\label{sec:org666b818}
...
the type of market. Germany, like many other countries, has a liberalized energy
system in which the generation and distribution of electricity are decoupled.
Multiple electricity markets exist in a liberalized energy system. They differ
in the auction design and in their reaction time between the order contract and
the delivery of electricity. Day-ahead markets and spot markets have a reaction
time between a day and several hours, whereas in operating reserve markets the
...
resources are considered in ascending order of the energy price until the
...
bids in the following form to the market: \((P, p_c, p_w)\), where \(P\) is the
amount of electrical power that can be supplied on demand in kW, \(p_c\) is the
capacity price for keeping the power available in \(\text{\euro{}/MW}\) and \(p_w\)
is the working price for delivered energy in \(\text{\euro{}/MWh}\). The TSO
...
to a merit order of the ascending \emph{working prices} \(p_w\). Hence, providers are
also compensated according to the actual power they supplied: \(R_w = p_w \times
P_a\), where \(P_a\) is the supplied power at the timeslot. Since provider get paid
according to their submitted price \(p_w\), instead of a market clearing price,
this type of auction is called \emph{pay-as-bid} auction.
\subsubsection{Spot Market}
\label{sec:orga85912a}
...
accessed 19\textsuperscript{th} February, 2019\label{orgefbbd20}}, but most electronic
...
trading volume of over 234TWh in 2018\textsuperscript{\ref{orgefbbd20}}. Participants place asks and bids
