...
\label{sec:orgd0d1bec}
\subsection{Research Motivation}
\label{sec:org5020d7a}
\begin{itemize}
\item \cite{lopes11_integ_elect_vehic_elect_power_system}
\end{itemize}
\subsection{Research Question}
\label{sec:org7f16338}
\subsection{Relevance}
\label{sec:org994c226}

\section{Theoretical Background}
\label{sec:orge40cc06}
\subsection{Electricity Markets}
\label{sec:orgbf8cfcf}
...
accessed 19\textsuperscript{th} February, 2019\label{org830d62d}}, but most electronic
...
trading volume of over 234TWh in 2018\textsuperscript{\ref{org830d62d}}. Participants place asks and bids
...
control reserve in the same period. For a detailed description about the market
design of balancing markets refer to Chapter \ref{sec-balancing-market}.

The bidding data consists of the electricity product, the offered capacity \(P\)
(MW), the capacity price \(p_c\) (\(\text{\euro{}/MW}\)), and the energy price \(p_e\)
(\(\text{\euro{}/MWh}\)) of each bid. Four different products are traded, which
are a combination of positive control reserve (feed electricity into the grid)
or negative control reserve (take electricity from the grid) and the provided
time segment (peak or non-peak hours). Since negative prices are allowed on the
secondary operating reserve market, the payment direction is included as well.
Moreover, information about the amount of capacity that was accepted, i.e.,
either partially or fully, is listed. Bids, which were not accepted by the TSOs
are not listed. An exemplary excerpt of the dataset is displayed in Table
\ref{data-operating-reserve}.
...
activated control reserve during that interval. For every 15-minute interval
within the given tender period of one week, the activated control reserve in
that interval was matched with the accepted bids in that tender period. At the
point where supply, i.e., offered capacity of bids, met demand, i.e., activated
control reserve, the critical price \(\overline{p_e}\) was determined.
...
respectively (cf. Table \ref{data-operating-reserve}). The critical capacity price
