...
\label{sec:orgba94b9e}
\paragraph{Policy}
\label{sec:org3335d94}
\begin{itemize}
...
full knowledge of the environment it is known as \emph{planning}, where optimal
solutions are \emph{computed}. In \emph{control} problems (Chapter \ref{sec-td-learning} and
\ref{sec-approximation}) optimal solutions are \emph{learned} from an unkown
environment.
...
\(\qpi(s,a)\) by \emph{greedily} taking the best short-term action \(a\!\in\!A\) at a given
...
\(\xrightarrow{\text{ I }}\) denotes a policy improvement step, and \(\pi_*\),
...
has to performed, which requires multiple sweeps through the state space. In
\emph{value iteration} the policy evaluation step is stopped after one sweep. In this
case, the two previous steps can be combined into one single update step:
\begin{equation}
\begin{split}
...
estimates, a procedure called \emph{bootstrapping}. In simple TD prediction, the
...
When a model is not available, it is useful to estimate \emph{action values},
...
value function for all \emph{state-action} pairs \(\qpi\), continual \emph{exploration}
needs to be ensured. On other words, the agent has to explore state-action pairs
which are seemingly disadvantageous given the current policy. This dilemma is
also called the \emph{exploration-exploitation} trade-off. One way to achieve
exploration are \emph{stochastic} policies. Stochastic policies have a non-zero
probability of selecting each actions in each state. A typical stochastic policy
is the \emph{\(\epsilon\)-greedy policy}, which selects the action with the highest
...
\caption[On-policy control with SARSA]{On-policy control with SARSA \cite{sutton18_reinf} \label{fig-sarsa}}
\end{figure}
...
policy to generate behaviour. The first policy is becoming the optimal policy by
evaluating the exploratory behaviour of the second policy. Off-policy approaches
are considered more powerful than on-policy approaches, and have a variety of
additional use-cases. On the other side, they often have a greater variance and
take longer time to converge to an optimum.
...
policy \(\pi\). This can be done similar to TD prediction of state values (Eq.
\ref{eq-td-prediction}). Instead of considering state transition, state-action
transitions are considered in this case. The update rule is constructed as
follows:
...
constituted the name SARSA. The according on-policy control step of the
algorithm is straightforward, and uses a \(\epsilon\)-greedy policy improvement, as
described in the previous paragraph. It has been shown that SARSA converges to
the optimal policy \(\pi_*\), when assuming infinite visits to all state-action
pairs.
...
\end{equation}
Here, the estimated action values \(Q\) are updated towards the best estimated
...
This chapter covered the most important RL methods. They are online methods,
learn from actual experience, and can be easily applied to real-world problems
with low computational effort. Moreover, the mathematical complexity of the
presented approaches is limited and they can be easiliy implemented into
computer programs. Temporal-Difference learning is a \emph{tabular} method, Q-values
are stored and updated in a lookup table. If state and action spaces are
continous or the number of states and actionsn is very high, a table
representation computational infeasible and the speed of convergence is
drastically reduced. In this case, a \emph{function approximator} can replace the
lookup table. The next chapter will cover approximation methods, which are used
to train such a function approximators.
\subsubsection{Approximation Methods \label{sec-approximation}}
\label{sec:org4d4ce61}
\paragraph{On-policy Prediction with Approximation}
\label{sec:orga688bce}
\begin{enumerate}
\item Linear Methods
\label{sec:orgc27b8ff}
\item Non-linear Methods: ANN
\label{sec:orgb4d6b3d}
\end{enumerate}
\paragraph{(On-policy Control with Approximation)}
\label{sec:orge60c159}
\clearpage
\section{Empirical Setting / Data (10\%)}
\label{sec:org0464b8a}
\subsection{Carsharing Fleets of Electric Vehicles}
\label{sec:orgb56f8c1}
\subsubsection{Raw Data}
\label{sec:org5311281}
