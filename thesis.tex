...
\label{sec:org058ded9}
\subsection{Research Motivation}
\label{sec:org0b599f0}
\begin{itemize}
\item \cite{lopes11_integ_elect_vehic_elect_power_system}
\end{itemize}
\subsection{Research Question}
\label{sec:org6e90d7d}
\subsection{Relevance}
\label{sec:org4d7d8fa}

\section{Theoretical Background}
\label{sec:org38cd74e}
\subsection{Electricity Markets}
\label{sec:orgc3ce6a9}
...
trading volume of over 234TWh in 2018\textsuperscript{\ref{orgc80eb6a}}. Participants place asks and bids for
...
\ref{fig-electricity-markets}). The day-ahead market plays an important role for
the integration of volatile renewable energy sources (RES) \cite{pape16_are_fundam_enoug}. Generators forecast
the expected generation capacity for the next day and sell those quantities on
the market \cite{karanfil17_role_contin_intrad_elect_market}. After the market closes, the
...
\cite{kiesel17_econom_analy_intrad_elect_prices}. In this way, RES generators
self-balance their portfolios, which is more cost effective than to rely on
balancing services, where the TSO imposes high balancing costs on participants
ex-ante \cite{pape16_are_fundam_enoug}.

On the \emph{EPEX Spot Intraday Continuous} market, electricity products can be traded
...
specified as price-quantity. When an order to buy (bid) and an order to
sell (ask) is matched, the trade immediately gets executed. The order book is
visible to all participants, hence it is known which unmatched orders exist at
the time of interest. With a market volume of 82TWh in 2018\textsuperscript{\ref{orgc80eb6a}}, the intraday market
has considerable less volume than the day-ahead market. Despite that, it plays
an important role for to the stability of the grid. All executed trades on the
intraday market potentially reduce the activation of control reserve through the
TSO.

Purchasing electricity on the continuous intraday market is attractive EV fleets
with uncertain mobility demand. Due to the short time before delivery, EV fleet
