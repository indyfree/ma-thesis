...
\label{sec:orgf0be0e1}
\subsection{Electricity Markets}
\label{sec:orgfad7e24}
\subsubsection{Balancing Market}
\label{sec:org7c342e4}
\subsubsection{Spot Market}
\label{sec:org36b42b4}
\subsection{Reinforcement Learning \label{reinforcement-learning}}
\label{sec:org2803c11}
...
\label{sec:org825151d}
\paragraph{Policy}
\label{sec:org8536219}
\begin{itemize}
...
The two most popular DP algorithms that compute optimal policies are called \emph{policy
iteration} and \emph{value iteration}. These methods perform "sweeps" through the
whole state set and update the estimated value of each state via an \emph{expected
update} operation. In \emph{policy iteration}, a value function for a given policy \(\vpi\)
needs to be computed first (\emph{policy evaluation}). A sequence of approximated
...
if the value \(\vpi(s)\) for a given state increases (\emph{policy improvement}). A way
...
converge to the optimum. This algorithm is called \emph{policy iteration}:
\begin{equation}
