...
electricity on the market and discharge EVs (sell electricity), when there is a shortage of electricity.
...
can mitigate risks and increase profits. Allocating EVs to different types of VPPs, that
...
VPPs and spot market VPPs, will be dynamically learned. To address changing
electricity price levels and customer demands over time we additionally propose
the use of an intelligent agent. The agent learns from historical data, its current environment
and realized profits to adjust its trading strategy dynamically.
...
We propose the use of a decision support system (DSS), which allocates idle EVs to different types of VPPs or to be available for renting.
