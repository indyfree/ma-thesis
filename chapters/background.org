...
well as SoC of the EVs. The authors formulated a MDP with discrete state and
...
of the environment's state. Values indicate the long-term desirability of a
state, that is the total amount of reward the agent can expect to accumulate
over the future, following a learned set of actions, called the policy. Policies
...
fully known, the learning problem is reduced to a planning problem (Chapter
...
unknown, and the agent's behavior has to be learned from experience (Chapter
