...
robust and scalable ML in industry and research cite:abadi16_tensor. We used
Google Colaboratory[fn:3] to train and evaluate the agent. This shared research
environment provides free access to computing resources that are optimized for
training ML models. More specifically, it provides a NVIDIA Tesla K80 GPU, with
2880 $\times$ 2 CUDA cores and 12GB GDDR5 VRAM . Additionally it is equipped
with a Intel(R) Xeon(R) CPU @ 2.30GHz (1 core, 2 threads), and over 12GB
available memory. Google Colaboratory can be used for 12 hours of consecutive
training, before loosing its state.
