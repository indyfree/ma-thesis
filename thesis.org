...
ref:agent-environment-interaction depicts the /Agent-Environment Interface/.
...
perceived by the agent as a representation of the environment, called /state/, $S_t
\in \S$. Based on the state, the agents selects an /action/, $A_t\in\A$, and
receives a numerical /reward/ signal, $R_{t+1}\in\R\subset\Re$, in the next
...
# TODO: Section Policies and Value Functions
# NOTE: State-Value Function
# NOTE: Action-Value Function
An essential task of almost every RL agent is, to estimate /value functions/.
...
future. Consequently, value functions are formulated with respect to a /policy/
...
\end{split}
\end{equation}
where $a\in\A$, $s,s'\in\S$, $r\in\R$. Equation ref:eq-bellman is the /Bellman Equation/.
