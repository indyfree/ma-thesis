...
So far, only tabular RL methods have been covered, which is the theoretical
foundation of RL in general. But in many real-world use cases the state
...
success, which coined the term /Deep Reinforcement Learning/
cite:mnih15_human_level_contr_throug_deep_reinf_learn,silver16_master_game_go_with_deep.
A simple feedforward ANN can be found in Figure ref:fig-ann. ANN have the advantage
...
the following:
/Eligibility traces/ offer a way to more general learning and faster convergence
...
Fitted-Q Iteration cite:ernst03_iterat combined Q-learning and /fitted value
...
in its the environment. /Actor-critic/ methods
