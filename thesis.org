...
with a total trading volume of approximately 567TWh in 2018, [fn:3] but most
...
# NOTE: Leave out since not imporant for our strategy??
On the day-ahead market, electricity is allocated by means of a sealed-bid double
auction. Bids and asks are sorted according to an ascending/descending merit
order of the submitted price respectively. At the point where supply quantities
(asks) meets demand quantities (bids), the clearing price is determined. All
participants are then uniformly compensated according to this price.
...
accurate intraday forecasts on the /intraday market/. This quantity called
/forecast error/ is used by RES generators to self-balance their portfolios.
