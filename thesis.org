...
#+CAPTION[Markov Decision Process]: The agent-environment interaction in a Markov decision process \protect\footnotemark label:agent-environment-interaction
...
When a state includes all information of /all/ previous states, the state
possesses the so-called /Markov property/. If not noted otherwise, the Markov
property is assumed throughout the thesis.
The dynamics function allows computing the /state-transition probabilities/,
...
The use of a /reward signal/ $R_t$ to formalize the agent's goal is a very
...
a /policy/ \pi. A policy is a mapping of states to actions, it describes the
...
The /state-value function of policy/ $\pi$ is denoted as $\vpi(s)$ and is
...
equation of the optimal value function $v_*$ is called the /Bellman
optimality equation/:
...
/Dynamic Programming/ (DP) is a method to compute optimal policies, the ultimate
...
full knowledge of the environment it is known as /planning/, where optimal
solutions are /computed/. In /control/ problems (Chapter ref:sec-td-learning and
ref:sec-approximation) optimal solutions are /learned/ from an unkown
environment.
...
$\qpi(s,a)$ by /greedily/ taking the best short-term action $a\!\in\!A$ at a given
...
/value iteration/ the policy evaluation step is stopped after one sweep. In this
...
look at /model-free/ prediction and /model-free/ control. As opposed to
...
TD learning, it is also categorized as /online learning/. Also note that TD
methods perform update operations on value estimates based on other learned
estimates, a procedure called /bootstrapping/. In simple TD prediction, the
...
/discount-rate/. Here, the update of value of a state is performed using the
observed reward $R_{t+1}$ and the estimated value $V(S_{t+1})$.

When a model is not available, it is useful to estimate /action values/,
instead of /state values/. If the environment is completely known, it is
...
estimated. This constitutes a problem, since not every /state-action/ pair will
...
value function for all /state-action/ pairs $\qpi$, continual /exploration/
...
also called the /exploration-exploitation/ trade-off. One way to achieve
exploration are /stochastic/ policies. Stochastic policies have a non-zero
...
#+CAPTION[On-policy control with SARSA]: On-policy control with SARSA \protect\footnotemark label:fig-sarsa
[[./fig/on-policy.png]]
...
lookup table. The next chapter will briefly cover approximation methods, which
are used to train such function approximators.
*** Approximation Methods label:sec-approximation
