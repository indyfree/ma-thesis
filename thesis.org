...
- 1. RQ: Monthly gross profit increase with benchmark strategy: Bar-Plot? (integrated naive
  strategy)
- Show profits by charging / loss according to lost rentals? Table
- Error Matrix? Predicted available EVs vs actually available --> Fixed by
  simulation! No result
- Decision outcomes: VPP, rental, available, MWh traded in each markets, (MWh
  charged regularly)

- Risk averse agent vs. Risk seeking agent/strategy?

# #+LATEX: \renewcommand\theadalign{bc}
# #+LATEX: \renewcommand\theadfont{\bfseries}
# #+LATEX: \renewcommand\theadgape{\Gape[2pt]}
# #+LATEX: \renewcommand\cellgape{\Gape[2pt]}


#+CAPTION[Bidding strategy outcomes]: Comparison of outcomes of naive bidding strategies over a 1.5 year period label:table-profits
#+ATTR_LATEX: :float sideways :align p{0.25\linewidth}|cccccc :placement [hp]
|------------------------------------------------------------------------------------------------+------------------+-----------+----------+------------+-------+-----------|
|------------------------------------------------------------------------------------------------+------------------+-----------+----------+------------+-------+-----------|
| Metric                                                                                         | Regular charging | Balancing | Intraday | Integrated | Risky | Benchmark |
|------------------------------------------------------------------------------------------------+------------------+-----------+----------+------------+-------+-----------|
| round(100 round(Energy bought(MWh) / (Energy bought(MWh) + Energy charged regularly(MWh)), 2)) |                0 |        39 |       47 |        100 |   100 |        91 |
| Energy bought (MWh)                                                                            |                0 |       803 |      985 |          1 |     1 |      1884 |
| Energy charged regularly (MWh)                                                                 |             2081 |      1278 |     1096 |          0 |     0 |       197 |
| Lost rental profits (\eur)                                                                     |                0 |         0 |        0 |          0 |       |         0 |
| No. Lost rentals                                                                               |                0 |         0 |        0 |          0 |       |         0 |
| Imbalances (MWh)                                                                               |                0 |         0 |        0 |          0 |       |         0 |
| Average electricity price ($\ekwh$)                                                            |             0.15 |         - |        - |          - |     - |         - |
| Gross profit increase (1000\eur)                                                               |               56 |           |          |            |       |      97.6 |
|------------------------------------------------------------------------------------------------+------------------+-----------+----------+------------+-------+-----------|
|------------------------------------------------------------------------------------------------+------------------+-----------+----------+------------+-------+-----------|
#+TBLFM: @2=round(100*round(@3/(@3+@4),2))
# ::@10=100* round((@9/17707.85),4)

\thead{Integrated \\ (risk seeking)}
\thead{Benchmark \\ (full information)}
