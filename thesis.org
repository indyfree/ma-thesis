...
the charging problem as a MDP, second, the developed learning algorithm is
explained, and third, implementation and training details are given.
...
observed state $\S$ every timestep. Bids are submitted every market period $h$,
thus the /timestep/ (also called iteration) of the RL problem is set to $h$. The
RL agent learns the optimal set of risk factors, by estimating a policy
$\pi(a|s)$ that maps every state $s\in\S$ to an action $a\in\A$.
...
$\vpphat{h}$ as the as the necessary amount of EVs to provide the predicted
charging power $\fPhat{}$ in time period $h$:
\begin{equation}
