...
2, 3, ..., $T$, the RL agent interacts with the environment. The latter is
...
The /dynamics/ of a MDP are defined by the probability that a state $s'\in
\S$ and a reward $r\in\R$ occurs, given the preceding state $s\in\S$ and
action $a\in\A$. In /finite/ MDPs, the random variables $\R_t$ and $S_t$ have
...
/dynamics/ of the MDP as following:
\begin{equation}
...
- Value function (of states, or state-action pairs)
- Policy:
- Optimal policy
