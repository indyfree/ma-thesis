...
# NOTE: Where to put changed market setup:
# - Minimum requirements
# - Different segments (2 weekly/ 6 daily)
...
to a merit order of the ascending /energy prices/ $p_e$. Hence, providers are
...
# NOTE: Mention: Estimate the critical prices, $p_c$ (not) and $p_w$ - in data
# section?
...
market until the market closes at 12pm on the day before delivery (Figure
...
cost effectivly self-balance their portfolios, instead of relying on balancing
...
specified as price-quantity. When an order to buy (bid) matches an order to sell
(ask), the trade immediately gets executed. The order book is visible to all
participants, hence it is known which unmatched orders exist at the time of
interest. The intraday market volume of 82TWh is considerably smaller than
day-ahead market's volume. Despite that, the intraday market plays a vital role
to the stability of the grid. All executed trades on the intraday market
potentially reduce the activation of control reserve through the TSO.
...
pushing for an energy turnaround since 2010  (German: /Energiewende/)[fn:4].
# TODO: Reformulate or cite
The high renewable energy content in the energy mix causes electricity prices to be
more volatile, which makes Germany an attractive location for the use of VPPs.

It has been argued that electric carsharing can simultaneously solve several
traditional mobility and environmental problems and are an important element of
future smart cities cite:firnkorn15_free_float_elect_carsh_fleet_smart_cities.
Further, it is widely regarded that the future of mobility will be electric,
shared, smart and eventually autonomous
cite:burns13_sustain_mobil,sterling18_three_revol. Carsharing providers are
already contributing to the first two points by operating large fleets of
electric vehicles. This research addresses the third point: Using electric
carsharing fleets to smartly participate in electricity markets. Carsharing
providers, like Daimler and BMW, operate their carsharing fleets in a free-float
model, which allows customers to pick up and drop vehicles at any place within
the operating zone of the provider. Customers pay by the minute and are offered
incentives to park the EVs at charging stations at the end of their trip.

# NOTE: Further description how data was collected? API, 5 minutes, etc..?
We obtained real-world trip data from Daimler's carsharing service Car2Go[fn:5]
that operates several large EV fleets in cities around the world.[fn:6]
...
have kindly been provided by ProCom GmbH[fn:7] for research purposes. In the
...
The dataset consists of trip data of around 500 Smart ED3 Fortwo vehicles in
...
In Table ref:data-car2go-raw the raw data is displayed, as we have obtained it
by Car2Go. The dataset contains spatio-temporal attributes, such as timestamp,
coordinates, and the address of the EVs at that point in time. Additionally,
status attributes of the interior and exterior are given (not displayed), and
most importantly the relative state of charge ($SoC$) and information whether
the EV is plugged into one of the 380 charging stations in Stuttgart. Note that
the data only contain EVs that are /available for rent/ in 5 minute intervals.
EVs which are parked at a charging station are also not available until they
have charged up approximately 70 SoC. Individual trips have to be reconstructed
using the GPS data of the cars. Following preprocessing steps have been taken:

1) Unused data columns were dropped
   ...
   - Interior, Exterior: :: Status attributes were not used in the analysis of
        this research. Although they could form interesting features for rental predictions.
   - Enging Type: :: All EVs in Stuttgart are electric vehicles.
2) The EV position coordinates were rounded to 4 decimals to reduce the GPS resolution to 10 meters.:

   The GPS accuracy of private industry sensors is usually around 5 meters under
   open sky and worse near buildings, bridges and trees[fn:8]. We identify trips
   by changing GPS locations of the EV (See next point). To not falsely identify
   trips, due to GPS measurement errors the resolution is reduced.
   # TODO: Example above and numbers?
3) Infer rental trips:

   If the position, according to the coordinates, change between two datapoints
   of the same EV, we infer that a customer rented the EV (cf. Table
   ref:data-car2go-raw, 3^{rd} to 4^{rd} row). Note, that we assume that Car2Go
   customer do not undertake trips that begin and end at the exact same
   location.
4) Infer charging stations:

   The data shows that EVs are parked at GPS locations where charging stations
   exist in reality, but they do not show up as /charging/ in the data. We
   assume that EVs, which are parked at charging stations are also plugged in.
   That is a valid assumption, since by German Law EVs have to be connected to a
   charging station if they are parked at one, and otherwise will be removed.
   Hence, the GPS location of EV is matched with GPS location of all charging
   stations in the dataset.
5) Clean data:
   - Service Trips :: *999* Trips were removed that had a trip duration longer than the
        maximum allowed rental time of two days. We assume that these trips were
        /service trips/ undertaken by Car2Go. Since in most of the time the EVs
        came back with a fully charged battery, the previous trip had to be
        altered to end at a charging station.
   - Incorrect charged EVs :: *number* EVs were removed that show incorrect
        charging behavior. These EVs showed an increase of more than 20% SoC
        between trips, while not being located at a charging station.
...
# Also for prediciton capacity has been calculated:



# TODO: How many charging stations in Stuttgart? Whitepaper says 380 how much
# does the data say?

Analyzing free-float trip data is substantially more difficult than fixed trip
data.
...
# - Relax 1 MW minimum bidding assumption -- Controller section
#   - 500kw
#   - 100kw

# TODO: Consistent wording: tender period = weekly, daily; segments, intervals, products, time windows
In this research, we use bidding data from the German secondary reserve market.
The following chapter will give an overview the dataset and preprocessing steps
that were taken. For a detailed description about the market design of balancing
markets refer to Chapter ref:sec-balancing-market.

# TODO: Also ecompasses activated control reserve data, remove footnote from
# next section
The data encompasses weekly lists of anonymized bids between 01.06.2016 and
01.01.2018. The electricity product, the offered capacity $P$ (MW), the capacity
price $p_c$ ($\text{\euro{}/MW}$) and the energy price $p_e$
($\text{\euro{}/MWh}$) of each bid is listed. Four different products are
...
the provided time segment (peak or non-peak hours). Since negative prices are
allowed on the secondary operating reserve market, the payment direction is
included as well. Moreover, information about the amount of capacity that was
accepted, i.e., either partially or fully, is listed. Bids, which were not
accepted by the TSOs are not listed. An exemplary excerpt of the dataset is
displayed in Table ref:data-operating-reserve.

#+CAPTION[Secondary Operating Reserve Market Data]: List of Bids of the German Secondary Reserve Market for the tender period 04.12.2017 - 11.12.2017. label:data-operating-reserve
...
operating reserve tenders will be possible in future energy markets. As
...
# NOTE: Upper bound (citation)
# NOTE: Move data sentence to top?
...
$\overline{p_c}$ and $\overline{p_e}$ are determined for each auctioned
interval. textcite:brandt17_evaluat_busin_model_vehic_grid_integ define
$\overline{p_c}$ as the capacity price of the bid that was just barely accepted,
whereas $\overline{p_e}$ is the highest energy price that was payed for
activated control reserve during that interval. The dataset was augmented with
data of /activated/ secondary control reserve in a 15-minute resolution.[fn:9]
For every 15-minute interval within the given tender period of one week, the
activated control reserve in that interval was matched with the accepted bids in
that tender period. At the point where supply, i.e., offered capacity of bids,
met demand, i.e., activated control reserve, the critical price
$\overline{p_e}$ was determined.
...
respectively (See Table ref:data-operating-reserve). The critical capacity price
$\overline{p_c} = 200.3 \text{ \!\euro{}/MW}$ is determined by the capacity
price of the last (third) accepted bid in that time segment. The TSO reported
that 18MW of control reserve were activated between 08:00 and 08:15. Hence, the
second bid determines the critical energy price $\overline{p_e} = 251
...
bidder would get compensated with:
$R = R_c + R_e =(10.73\text{ \!\euro{}/MW}\times15\text{MW}) + (251 \text{
\!\euro{}/MWh}\times13\text{MW}\times 0.25\text{h}) = 976.7\text{ \!\euro{}}$
Note that the second bidder get compensated for 13MW instead of the submitted
15MW, since in total only 18MW of control capacity get activated, which were
partly fulfilled by the 5MW of the first bidder.
...
# data have been provided by ProCom GmbH[fn:1] and encompass order books and
...
# delivery, which is beneficial for our proposed trading strategy.
...
[fn:4] Energy Concept for an Environmentally Sound, Reliable and Affordable energy cite:economics10_energ_concep_envir_sound_reliab_affor

[fn:5] https://www.car2go.com

[fn:6] https://www.car2go.com/media/data/germany/microsite-press/files/car2go_white-paper_electric-mobility_2018.pdf,
accessed 21^{th} February 2019.

[fn:7] https://procom-energy.de/

[fn:8] See https://www.gps.gov/systems/gps/performance/accuracy/, accessed
23^{th} February 2019.

[fn:9] The data of activated secondary control reserve is freely available at https://regelleistung.net.

[fn:10] https://www.regelleistungen.net

[fn:11] https://www.epexspot.com
