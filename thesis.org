...
As displayed in Table ref:data-car2go-raw, the data contain spatio-temporal
attributes, such as timestamp, coordinates, and address of the EVs.
Additionally, status attributes of the interior and exterior are given, the
relative state of charge ($SoC$) and information whether the EV is plugged into
one of the 380 charging stations in Stuttgart.
...
#+ATTR_LATEX: :float sideways :align cc|ccccc :placement [hp]
...
data of /activated/ secondary control reserve in a 15-minute resolution.[fn:8]
