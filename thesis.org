...
\begin{equation*}
    \cup{} \defeq \min_{t \in \cal{T}} p^u_t
\end{equation*}
...
| $\cp$       | Capacity price offered on the balancing market.                           | $\text{\euro{}/MW}$  |
| $\ep$       | Energy price offered on the balancing market.                             | $\text{\euro{}/MWh}$ |
...
| $\up$       | Unit price offered on the intraday market.                                | $\text{\euro{}/MWh}$ |
...
2) The controller predicts the clearing prices

#+BEGIN_SRC python :results output :exports none :var h=15
print("Number of control periods in a day: %d " % ((60 / h) * 24))
#+END_SRC

#+RESULTS:
: Number of control periods in a day: 96
