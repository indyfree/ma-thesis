...
will only observes returns for one of the actions. In order to evaluate the
...
probability of selecting all actions in each state.
# NOTE: Sarsa: On-policy TD Control
- On-policy ($\epsilon$-greedy policy)
- Continually estimate $\qpi$ for the behaviours policy \pi and change \pi
