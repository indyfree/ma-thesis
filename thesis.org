...
- DP, TD, Monte Carlo all use some variation of GPI.
# TODO: Label as "Planning"
...
model of the environment (e.g., known /state-transition probabilities/, Eq.
...
\end{equation}
where $\xrightarrow{\text{ E }}$ denotes a /policy evaluation/ and
$\xrightarrow{\text{ I }}$ denotes a /policy improvement/ and $\pi_*$ and
$\vstar$ are the optimal policy and value function, respectively.
# TODO: Add greedy-policy and strict improvement
...
space. In /value iteration/ the policy evaluation step is stopped after one
...
# TODO: Section Monte Carlo Prediction, Estimation and Control
...
estimates, a procedure called /bootstrapping/. In a simple TD prediction method,
...
value function for /state-action/ pairs $\qpi$, continual /exploration/ needs to
...
called the /exploration-exploitation/ trade-off. One way to achieve exploration
is to only consider /stochastic/ policies. Stochastic policies have a non-zero
...
\epsilon-greedy action selection. /Off-policy/ methods improve the deterministic
...
behaviour of the second policy. /Off-policy/ approaches are considered more
powerful than /on-policy/ approaches, and have a variety of additional
use-cases. On the other side, they often have a greater variance and take longer
time to converge to an optimum.
...
ref:eq-td-prediction). Instead of considering state transition, transitions from
state-action pair to state-action pair are considered in this case. The update
...
# NOTE: Q-learning: Off-policy TD Control
# NOTE: Explain off-policy i.e. cite:sutton18_reinf p.103
