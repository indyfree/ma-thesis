...
Hence, providers are also compensated according to the actual energy they
supplied: $R_e = p_e \times E^{bal}_t$, where $E^{bal}_t$ is the supplied energy
at the timeslot $t$. Since provider get paid according to their submitted price
$p_e$, instead of a market clearing price, this type of auction is called
/pay-as-bid/ auction.
...
presented. For the sake of conciseness and accuracy, we formulate the problem as
/controlled EV charging/: The EV fleet operator represents the controller, which
optimally charges the fleet by providing negative balancing capacity.


#+CAPTION[Table of Notation]: Table of Notation label:table-notation
#+ATTR_LATEX: :environment longtable :align c|c|c :placement [hp]
|-------------------------+----------------------------------------------------------------+----------------------|
|-------------------------+----------------------------------------------------------------+----------------------|
| Variable                | Description                                                    | Unit                 |
|-------------------------+----------------------------------------------------------------+----------------------|
| $P^{bal}$               | Amount of power to offer on the balancing market               | kW                   |
| $E^{bal}_t$             | Amount of supplied energy to balancing market                  | MWh                  |
...
|-------------------------+----------------------------------------------------------------+----------------------|
| $Q^{fleet}_t$           | Amount of available fleet charging capacity at time $t$        | kWh                  |
| $P^{fleet}_t$           | Amount of available fleet charging power at time $t$           | kW                   |
| $\widehat{P}^{fleet}_t$ | Predicted amount of available fleet charging power at time $t$ | kW                   |
|-------------------------+----------------------------------------------------------------+----------------------|
| $t$                     | Control period                                                 | -                    |
| $\Delta t$              | Length of control period                                       | minutes              |
| $T$                     | Total number of control periods in day                         | -                    |
| $h$                     | Market period                                                  | -                    |
| $\Delta t$              | Length of market period                                        | minutes              |
| $H$                     | Total number of market periods in day                          | -                    |
| $N_t$                   | Number of control periods in a market period                   | minutes              |
|-------------------------+----------------------------------------------------------------+----------------------|
|-------------------------+----------------------------------------------------------------+----------------------|
...
# NOTE: cite:avci18_manag_elect_price_model_risk ?
1) The controller is able to forecast the mobility demand of the EV fleet at
   ...
   estimate the amount of plugged-in EVs and consequently the charging capacity $Q_t$ of
   the fleet at t. with increasing certainty, one week,
   24 hours and 30 minutes ahead,
...
2) The controller is able to forecast electricity prices of spot and balancing
   ...
   algorithms cite:weron14_elect_price_forec.
Assuming available accurate forecasting information is common practice in the
...
** Sensitivity Analysis: Bidding Strategyl
* Discussion
# NOTE: 5%
** Generalizability
