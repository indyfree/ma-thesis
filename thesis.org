...
for keeping charging power available, only for the actual energy charged. When
the costs for charging from the balancing market one week ahead $\Cb{h + (H
\times 7)}(\fPhat{h + (H \times 7)})$ are higher than the costs of charging from
the intraday market at the same market period $\Ci{h + (H \times 7)}(\fPhat{h +
(H \times 7)})$, the controller does not place bids on the balancing market and
continues to Decision Phase II.

# NOTE: New section/heading?
In a third step, the controller has to take a decision on the amount of
balancing power it should submit to the balancing market. Determining the
bidding quantity is a central piece of the controlled charging problem. The
controller needs to balance the risks of 1) Not procuring enough energy from the
market to charge the fleet and 2) Procuring more energy that it can charge. In
this case the fleet is facing costs of compromising customer mobility or worse,
imbalance costs from the markets. Renting out EVs is much more profitable than
using EVs as a VPP to participate on the electricity markets. Refusing customer
rentals, in order to fulfill market commitments, induces opportunity costs of
foregone rentals $C^{rental}$ for the fleet. Imbalance costs occur in the worst
case, when the fleet can not charge the committed amount energy at all, even with
denying rentals.

Because the predicted charging power $\fPhat{}$ faces more uncertainty with an
increased time horizon, commitments to the balancing market one week-ahead bare
more risks than procuring on the intraday market 30 minutes ahead.
...
\begin{equation}
  ...
  \begin{cases}
    ...
    \fPhat{h}\!-\!\Pb{h}\!-\!\li{h}, & \text{otherwise}
  \end{cases}
\end{equation}

1) Light-grey: Determine bidding quantity, Chapter ref:sec-model-rl
