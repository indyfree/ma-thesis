...
quantities $\Pb{h}$, $\Pi{h}$ to submit to the markets each market period $h$.
The RL agent decides on the risk factors (i.e., takes an action) based on the
observed state $S_t \in \S$ every timestep $t\!=\!h$. It does so, by learning a
policy $\pi(a|s)$ that maps every state $s\in\S$ to an action $a\in\A$.
*** Markov Decision Process Definition

The state space is compromised of the observed information of the environment
the agent can use to decide on the action it is going to take. The bidding risk
is intuitively dependent on the hour of the day of the considered market period.
During the daytime, more customer activity is expected, resulting in an
increased uncertainty of the available charging power. Additionally the bidding
risk is dependent on the current and predicted amount of charging power. (Why?)
The more available charging power the higher the chance that any
"over-commitment" can be substituted by the rest of the VPP.


When considering the current and the predicted amounts of available charging
power, $\fP{t}$, $\fPhat{t+2}$, $\fPhat{t+(7\!\times\!H)}$ we arrive at a very
high dimensionality of states that compromises the feasibility of the RL problem.
We address the state explosion problem by defining $\vpphat{}$ as the necessary
amount of available EVs to provide the predicted charging power $\fPhat{}$ in time
period $t$:
\begin{equation}
...
\begin{equation}
    \S \defeq \left\{h(t), \vpp{t}, \vpphat{t+2}, \vpphat{t+(7\!\times\!H)}\right\}
\end{equation}
where:
...
# - $\fPhat{h+2}$ is the predicted amount of available charging power 30 minutes
#   ahead, with discrete values in the range $\left[0,\;|\F|\!\times\!\gamma\right]$
#   of 0.1kW increments.
# - $\fPhat{h+(7\!\times\!H)}$ is the predicted amount of available charging power
#   one week ahead, with discrete values in the range
#   $\big[0,\;|\F|\!\times\!\gamma\big]$ of 0.1kW increments.

- See cite:reddy11_strat for State/Action notation
...
\end{equation}
where:
- $\lb{t}$ is the risk factor for bidding on the balancing market, with discrete
  values in the range $\big[0,1\big]$ in 0.1 increments.
- $\li{t}$ is the risk factor for bidding on the intraday market, with discrete
  values in the range $\big[0,1\big]$ in 0.1 increments.
