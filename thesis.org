...
a /policy/ \pi. The policy is a mapping of states to actions, it describes the
...
The /state-value function of policy/ $\pi$ is denoted as $\vpi(s)$ and is defined
as the expected return when starting in $s$ and following policy $\pi$ (Eq.
ref:eq-state-value). The /action-value function of policy/ $\pi$ is denoted as
...
other policies.  The /optimal state-value function/ is defined as following:
\begin{equation}
    \vstar(s) \defeq \max_a \vpi(s), \text{ for all } s\in\S
\end{equation}
A similar definitions hold for the /optimal action-value function/ $\qstar$.
...
RL history. Most RL methods are /approximately/ solving the Bellman optimality
...
*** Tabular Methods
# TODO: Section Dynamic Programming: Policy Iteration, Value Iteration
