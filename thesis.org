...
by textcite:tomic07_using_fleet_elect_drive_vehic_grid_suppor, with more
...
a the states. textcite:peters13_reinf_learn_approac_to_auton build on that work
...
market. Similar to textcite:kahlen18_elect_vehic_virtual_power_plant_dilem the
...
textcite:kahlen18_elect_vehic_virtual_power_plant_dilem don't consider imbalance
cost in their model and avoid them by always sacrificing EV mobility in order to
balance the market. textcite:vandael15_reinf_learn_heuris_ev_fleet use a
...
same transformer. The RL agents learned to charge at minimal costs,
without overloading the transformer. textcite:dusparic13_multi utilized
/W-Learning/ to learn multiple policies (i.e., objectives like
ensuring minimum battery charged or ensuring charging at low costs) at the same
time. textcite:taylor14_accel_learn_trans_learn extended this research by
...
learn their bidding strategy with standard /Q-Learning/ and discrete state and
action spaces. textcite:giorgio13_elect_vehic presented a multi-agent solution to
...
the authors employ standard /Q-Learning/ and the \epsilon-greedy approach for
action selection. textcite:vaya14_optim also proposed a multi-agent approach,
...
to learn their optimal bidding strategy. The strategy, builds on the agents
...
and privacy problems. textcite:shi11_real consider a V2G control problem
...
real-time price signals of the market. It is a single-agent approaches where the
action space only compromises the control actions charging, discharging and
regulation. textcite:chis16_reinf_learn_based_plug_in looked at reducing costs
of charging for single EVs using known day-ahead prices and predicted next-day
prices. For prediction a Bayesian ANN was employed and /fitted Q-Learning/ was
...
of the EVs. The authors formulate a MDP with discrete state and action spaces
...
#   parencite:dauer13_market_based_ev_charg_coord,taylor14_accel_learn_trans_learn
** Bidding Strategy
# NOTE: Assume aggregator is a price taker
** Dispatch Heuristic / Algorithm
# - See parencite:vandael15_reinf_learn_heuris_ev_fleet
