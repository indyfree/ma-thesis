...
essence, we propose a solution how EV fleet providers can utilize a portfolio of
VPPs to profitably provide balancing services to the grid on multiple markets.
The model uses an RL agent that learns an optimal bidding strategy by
interacting with the electricity markets reacts to changing rental demand of the
EV fleet rental. A control mechanism bids on electricity markets, allocates
available EVs to VPPs and intelligently dispatches EVs to provide the charging
capacity. This chapter is structured as follows: First, the information
assumptions are listed, and the general control mechanism is explained.
Afterwards, the bidding strategy and the RL approach is described in detail.
Finally, the dispatch algorithm is presented. For the sake of conciseness and
accuracy, we formulate the problem as /controlled EV charging/: The EV fleet
operator represents the controller, which optimally charges the fleet by
providing negative balancing capacity.

** Required Information Assumptions
In order to optimize the controllers VPP bidding strategy, certain information
is assumed to be available.

# TODO: Mathematical symbol for available battery capacity.
# Use $Q$ and $\hat q$?
1) The controller can forecast the mobility demand of its EV fleet one week, 24h
   and 30 minutes ahead with decreasing accuracy based on historical data. More
   specifically, it can estimate the amount of available battery capacity
   $Capacity_todo$ of plugged-in EVs to be used for the VPP.

   cite:kahlen18_elect_vehic_virtual_power_plant_dilem,kahlen17_fleet,wagner16_in_free_float
   successfully showed this using a time-based sinusoidal model for a day-ahead
   prediction and a random forest model for a week-ahead prediction.
2) The controller can estimate the loads and prices of spot and balancing
   markets based on historical data.
   - cite:weron14_elect_price_forec
   - cite:avci18_manag_elect_price_model_risk
   - cite:brandt17_evaluat_busin_model_vehic_grid_integ,vandael15_reinf_learn_heuris_ev_fleet?
   - cite:mashhour11_biddin_strat_virtual_power_plant_1
   - Review: See proposal

Vandael: In this paper, we assume predictions of day-ahead prices
are available, which is supported by well-advanced day-ahead
price prediction methods [20].

3) EV fleet charges at a fixed industry tariff otherwise
4) Battery depreciation not considered
** Control Mechanism
2 Phases:
1) Decision Phase I: Day-ahead/Week-ahead
1) Decision Phase II: Intraday
2) Real-time / Allocation phase?
