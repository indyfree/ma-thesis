...
# NOTE: Model-free Prediction
# NOTE: TD(0) Prediction
...
When a model is not available, it is useful to estimate the /action values/,
...
powerful than the simpler /on-policy/ approaches. On the other side they require
additional concepts and are slower to converge.

# NOTE: Sarsa: On-policy TD Control
A popular /on-policy/ TD Control method is SARSA.
...
- Sarsa converges to an optimal policy as long as all state-action pairs are
...
- off-policy (/target/ (deterministic) and /behaviour/ policies (stochastic, exploratory), importance sampling)
