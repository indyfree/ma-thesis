...
\begin{equation}
    V\!P\!P \defeq \{i\;|\;i\in\F \vee \c = 1 \vee \Omega\!-\!\omega_{i}\!\geq\!\gamma\Delta{t}\} \text{ ,}
\end{equation}
...
the charging problem as a MDP, second, the used learning algorithm is explained,
and third, implementation and training details are given.
...
action) based on the observed state $S_t \in \S$ every timestep $t$. Since the
controller can submit bid every market period $h$, $t$ is chosen to be $h$.
The agent decides on the risk factors, by learning a policy $\pi(a|s)$ that maps
...
The state space compromises the observed information of the environment the
agent uses to decide on the action it is going to take. The bidding risk is
naturally dependent on the hour of the day of the considered market period. For
example, the increased customer activity during rush hour results in increased
uncertainty of the available charging power. Additionally, the current and
estimated (future) size of the VPP ($\vpp{}$) can influences the bidding risk.
Large VPPs benefit from the /risk-pooling/ effect cite:kahlen17_fleet.
Intuitively that means, that larger VPPs have an increased probability that
"lost" charging power, due to unforeseen rentals, can be substituted by the rest
of the VPP.

Since forecasts of available charging power are already available, we define
$\vpphat{t}$ as the as the necessary amount of EVs to provide the predicted
charging power $\fPhat{}$ in time period $t$:
\begin{equation}
    \vpphat{t} \defeq \left\lceil\frac{\fPhat{t}}{\gamma}\right\rceil \text{ ,}
\end{equation}
where $\gamma$ is the charging power per EV.

# FIXME: cited reddy
The state space is the set defined by all valid values of the elements of the
following tuple:
\begin{equation}
    \S \defeq \left\langle h(t), \vpp{t}, \vpphat{t+2}, \vpphat{t+(7\!\times\!H)}\right\rangle
\end{equation}
where:
- $h(t)$ is the current daytime in hours, with discrete values in the range
  $\big[0,\;23\big] \in \Ne$.
- $|VPP|_t$ is the current VPP size, with discrete values in the range
  $\big[0,\;|\F|\big] \in \Ne$.
- $\vpphat{t+2}$ is the predicted VPP size 30 minutes ahead, with discrete values in the range
  $\big[0,\;|\F|\big] \in \Ne$.
- $\vpphat{t+(7\!\times\!H)}$ is the predicted VPP size 7 days ahead, with discrete
...
$\lb{t},\li{t}$ constitute the action space:
\begin{equation}
    \A \defeq \left\{\lb{t},\li{t} \in \Re_{0 \leq \lambda \leq 1} \right\}
\end{equation}
where:
- $\lb{t}$ is the risk factor for bidding on the balancing market 7 days ahead,
  with discrete values in the range $\big[0,1\big]$ in 0.05 increments.
- $\li{t}$ is the risk factor for bidding on the intraday market 30 minutes
  ahead, with discrete values in the range $\big[0,1\big]$ in 0.05 increments.
The action space encompasses $4\!\times\!10^2 = 400$ actions. The state space
and action space were consciously discretized to facilitate fast convergence
rates. (Cite? More info?)
...
*** Learning Algorithm
- cite:dauer13_market_based_ev_charg_coord,
- cite:di13_elect_vehic
- cite:vaya14_optim
...
- Deep double Q learning
  ...
  - Image: cite:wang15_duelin_networ_archit_deep_reinf_learn
