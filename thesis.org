...
| $\omega_{i}$              | Amount of electricity stored in EV $i$.                                   | $\kwh$  |
| $\Omega$                  | Maximum battery capacity of EVs.                                          | $\kwh$  |
| $\delta$                  | Charging power of EVs at the charging station.                            | $\kw$   |
|---------------------------+---------------------------------------------------------------------------+---------|
...
market period $h$, that minimize the cost of charging, while avoiding the risks
of lost rental profits $\oc$ or imbalance costs $\beta_h$. The total
...
The costs for charging $\Cb{h}(\Pb{h})$, $\Ci{h}(\Pi{h})$ are clearly dependent
on the chosen risk factors $\lb{h}$, $\li{h}$ (see Eq. ref:eq-model-pb, Eq.
ref:eq-model-pi; dependence omitted in notation). In summary, the problem can be
...
detect how many EVs are eligible to be used as VPP. EVs are eligible if they (a)
are connected to a charging station ($\c$), and (b) have enough free battery
storage available ($\Omega\!-\!\omega_{i}$) to charge the next control period.
Hence, the VPP is defined as:
\begin{equation}
...
react to unforeseen rentals. If a customer want to rent out an EV that is
...
controller as to account for lost rental profits $\oc$.
...
/16.08.2017-15:00/. It predicts that at that point in time 250 EVs are connected
...
RL agent determined the risk of bidding on the balancing market at
$\lb{h}\!=\!0.3$. Consequently, the controller sets the bidding quantity to
$\Pb{h}\!=\!\fPhat{h}\!\times\!(1\!-\!\lb{h})\!=\!900\!\times0.7\!=\!630\kw$ and
...
low remaining risk of $\li{h}\!=\!0.05$, and sets the bidding quantity to
...
period. It assigns 223 EVs to the VPP to provide the committed 801kW charging
power for the market period time $\Delta h$ of 15 minutes. During that time,
three customers want to rent out EVs, which are allocated to the VPP. The first
two rentals are accepted, because the two other available EVs can be allocated
to the VPP. The third rental has be to refused, since no other EV is available
to charge the procured energy. The controller has to account for the opportunity
costs of the lost rental $\rho_{t,h}$.
# TODO: Update Account with lost rentals.
