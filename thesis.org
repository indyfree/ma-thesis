...
Bids are submitted every market period $h$, thus the /timestep/ (also called
iteration) of the RL problem is set to $h$. The RL agent decides on the risk
factors (i.e., takes an action) based on the observed state $S_h \in \S$ every
timestep $h$. It does so, by learning a policy $\pi(a|s)$ that maps every state
$s\in\S$ to an action $a\in\A$.
