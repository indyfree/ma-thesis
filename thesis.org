...
- Learning from actual experience, Requires no prior knowledge of
  environment/model but can still achieve optimal behaviour
- Method to *learn* from partial returns (as opposed to averaging over complete
  returns from episode, as in MC, and opposed to *compute* value function in DP
  when full knowledge of MDP available)
- When model not available it is useful to estimate /action/ values $\qstar$. In
  ...
  /state-action/ pair, as opposed to just the state. That is a problem, since
  not every /state-action/ pair will be visited when following a deterministic
  ...
  $\pi(a|s)$ returns exactly one action, given the current state.) /Exploration/
  needs to be maintained. In order to evaluate the value function for
  /state-action/ pairs $\qpi$, continual exploration needs to be ensured. One
  way to achieve exploration is to only consider /stochastic/ policies.
  Stochastic policies have a non-zero probability of selecting all actions in
  each state.

- On-policy ($\epsilon$-greedy policy) vs off-policy (/target/ (deterministic)
  and /behaviour/ policies (stochastic, exploratory), importance sampling)
...
$t\!+\!1$ they immediately make an update using the observed reward $R_{t+1}$ and the
estimate $V(S_{t+1})$, as opposed to wait till the end of the episode to observe
the occurred return.
\begin{equation}
    V(S_t) \leftarrow V(S_t) + \alpha\big[R_{t+1}+\gamma V(S_{t+1}) - V(S_t)\big]
\end{equation}
- called /TD(0)/ or /one-step TD/ update the value of a state using the observed
