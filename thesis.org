...
the risk factors $\li{h}$, $\lb{h}$, which determine the bidding quantities
$\Pb{h}$, $\Pi{h}$ to submit to the markets each market period $h$. The RL agent
decides on the risk factors (i.e., takes an action) based on the observed state
$S_t \in \S$ every timestep $t\!=\!h$. It does so, by learning a policy $\pi(a|s)$ that
