...
# Add: profits that can be made from charging at a cheaper price than industry tariff.
...
/control periods/ $t$ are typically shorter than market periods. In our case,
...
committed balancing capacity during all control periods $t$ of the market period
$h$. In order to optimally place bids on the balancing market the controller has
to take the following steps:

# TODO: Predicted not mentioned
...
the number of EVs that are connected to a charging station, with enough battery
capacity to charge the next control period $t\!+\!1$. To minimize the risk of not
being able to provide the committed capacity, and consequently causing imbalance
costs, the predicted fleet charging power in a market period is defined as the
minimal predicted fleet charging power of all control periods in a market period
($N_t$).
\begin{equation}
...
balancing and intraday market.  As mentioned in the Chapter
ref:sec-model-assumptions, the critical prices $\ccp{}, \cep{}, \cup{}$ are
assumed to be available for all market periods. The cost function for charging
...
of an hour $\frac{\Delta h}{60}$. Note that the critical energy price
$\cep{}\!\in\!\Re$, can also take negative values, resulting in profits for the
fleet, while the critical capacity price $\ccp{}\!\in\! \Re^+_0$ is can not take
negative values and therefore never results in costs for the fleet. The cost
...
needs to balance the risks of (a) not procuring enough energy from the market to
charge the fleet, and (b) procuring more energy that it can charge. In the
latter case the fleet is facing costs of compromising customer mobility, or
worse, high imbalance costs from the markets. Renting out EVs is much more
...
opportunity costs of lost rentals $\rho$ on the fleet. Imbalance costs $\Phi$
occur, when the fleet can not charge the committed amount energy at all, even
with denying rentals. The controller faces additional risks by bidding on the
balancing market, as the predictions of available charging power include more
uncertainty one week ahead than on the intraday market, 30 minutes ahead. To
account for the mentioned risks, we introduce a /risk factor/ $\lambda \in
\Re_{0 \leq \lambda \leq 1}$, where $\lambda = 0$ indicates no risk, and
$\lambda = 1$ a high risk.
...
charge cheaper from the intraday market or with the regular industry tariff
$p^{i}$, it does not place a bid. Otherwise, the controller submits $\Pb{h}$ to
the market.
...
delivery on the intraday market. In this phase, the controller places bids
for the market period $h+30$.
# TODO: Generalilzation, lead time, trades always get matched, prices?
...
\begin{equation}
  \Pi{h}=
  \begin{cases}
    0, & \text{if}\ \Ci{h}(\fPhat{h}) \geq \Ei{h} \times p^{i}\\
    (\fPhat{h}\!-\!\Pb{h}) \times (1\!-\!\li{h}), & \text{otherwise}
  \end{cases}
\end{equation}
Note that any amount of electricity that the controller procured from the
balancing market, does not need to bought from intraday market for the same
market period. Because the predicted charging power $\fPhat{h}$ is expected to
be  more accurate 30 minutes ahead, the controller is able to correct "bidding
errors" it made in the first decision phase.
...
# TODO: What values can \theta_{\lambda} take?
# TODO: Cost *functions* or not, make depended on lambda??
# TODO: = or \defeq?
\begin{equation}
    \Cf{}(\theta_{\lambda}) \defeq \sum^{N_h}_h
    \bigg[ \Cb{h}(\Pb{h}) + \Ci{h}(\Pi{h}) + \beta_{h}
    + \sum_t^{N_t} \sum_i^{N_f} \rho_{t, i} \bigg] \text{ ,}
\end{equation}
where \theta_{\lambda} is the set of all risk factors $\lb{h}$, $\li{h}$ for all
considered market periods $N_h$. The opportunity costs of denying a rental of EV
$i$ in control period $t$ is denoted with $\rho_{t, i}$. The costs for charging
$\Cb{h}$, $\Ci{h}$ are dependent on committed charging power $\Pb{h}, \Pi{h}$.
The charging power is dependent on the chosen risk factors $\lb{h}$, $\li{h}$
that are omitted here for convenience of notation. In summary the problem can be
formulated as minimizing the total costs of the fleet, by choosing the optimal
set of parameters $\theta_{\lambda}$ in all market periods $h \in \cal H$.
\begin{equation}
\begin{aligned}
...
# NOTE: Similar to cite:vandael15_reinf_learn_heuris_ev_fleet

/Example:/ At 3pm on the 9^{th} of August 2017, the controller enters its first
decision phase for procuring electricity one week ahead, the market period $h$ =
/16.08.2017-15:00/. First, it predicts that at that point in time, 250 EVs are
...
$\fPhat{h}$ = 810kW, for the same market period /16.08.2017-15:00/, this time
...
The controllers goal is to choose the risk factors $\lb{h}$, $\li{h}$ for every
market period, that minimize the cost of charging, while avoiding the risks of
lost rentals or imbalance costs.

# TODO: What values can \theta_{\lambda} take?
# TODO: Cost functions or not, make depended on lambda??
\begin{equation}
    \Cf{}(\theta_{\lambda}) \defeq \sum^{N_h}_h \bigg[ \Cb{h} + \Ci{h} + \Phi_{h}
    + \sum_t^{N_t} \sum_i^{N_f} \rho_{t, i} \bigg] \text{ ,}
\end{equation}
where \theta_{\lambda} is the set of all risk factors $\lb{h}$, $\li{h}$ for all
considered market periods $N_h$.
...
#    & & \text{for} \lb{h}, \li{h} \in \theta_{\lambda}, \; h = 1, \ldots, N_h.


Choosing a optimal $\lambda$ for each market is a difficult task, which we solve by using a RL learning approach,
explained in Chapter ref:sec-model-rl.


/Example:/ Given, $\Cb{h}$, $\Ci{h}$, p^i
...
Mechanism:
- Only EVs with enough battery capacity left to charge the next control period
...
*** Dispatch Phase
# When no replacement for VPP is available, we loose rental profits and also
# violate customer mobility
...
# NOTE: What about imbalance costs $\lambda^+$
\begin{equation}
